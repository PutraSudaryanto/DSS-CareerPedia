...
class Users extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('level_id, password, username, enabled, verified, deactivate, invisible,
				old_password, new_password, confirm_password, invite_code, reference_id, statistic', 'safe'),
			array('
	...
	public function search()
	{
		...
		$criteria->compare('first_name',strtolower($this->first_name),true);
		$criteria->compare('last_name',strtolower($this->last_name),true);
		$criteria->compare('displayname',strtolower($this->displayname),true);
		...
		$criteria->compare('deactivate',$this->deactivate);
		$criteria->compare('search',$this->search);
		$criteria->compare('invisible',$this->invisible);
		$criteria->compare('show_profile',$this->show_profile);
		$criteria->compare('privacy',$this->privacy);
		$criteria->compare('comments',$this->comments);
		$criteria->compare('last_email',strtolower($this->last_email),true);
	...
	protected function beforeValidate() 
	{
		...
		if(parent::beforeValidate()) {		
			if($this->isNewRecord) {
				...
				$this->profile_id = 1;
				$this->language_id = OmmuLanguages::getDefault();
				...
				$this->locale_id = OmmuLocale::getDefault();
				$this->timezone_id = OmmuTimezone::getDefault();
	...
	protected function afterSave() 
	{
		...
		if ($this->verified == 0) {
			...
			if($this->isNewRecord) {
				$verify->verify_date = $this->creation_date;
			}
			$verify->save();
		}
		...
		if($this->isNewRecord) {
			...
			if ($setting->site_type == 1 || ($setting->site_type == 0 && $this->level_id != 1)) {
				...
				$option->invite_limit = $setting->signup_numgiven;
				$option->save();
				
				// Add and Update Subscribe Newsletter
				$newsletter = SupportNewsletter::model()->findByAttributes(array('email' => $this->email), array(
					//'select' => 'id, user_id, email',
				));
				if($newsletter != null && $newsletter->user_id == 0) {
					$newsletter->after_register = 1;
					$newsletter->user_id = $this->user_id;
					if($newsletter->save()) {
						$this->statistic[] = 3;
					}
				} else {
					$subscribe = new SupportNewsletter;
					$subscribe->after_register = 1;
				...
				if($invite != null && $invite->member_id == 0) {
					...
					$invite->reference_id = $setting->signup_checkemail == 1 ? $this->reference_id : UserInvites::getInvite($this->email, 'desc')->user_id;
					if($invite->update()) {
						$this->statistic[] = 4;
					}
				}
		...
		if(!empty($this->statistic)) {
			$arrayStatistic = array_unique($this->statistic);
			foreach($arrayStatistic as $val) {
				$criteria=new CDbCriteria;
				if($val == 1) {
					$select = 'signup_from_public';
				} else if($val == 2) {
					$select = 'signup_from_public';
				} else if($val == 3) {
					$select = 'signup_newsletter_success';
				} else if($val == 4) {
					$select = 'signup_invite_success';
				} else if($val == 6) {
					$select = 'forgot_password_success';
				}
				$criteria->select = $select;
				$statistic = UserStatistics::model()->findByPk(Yii::app()->session['statistic_current_date'], $criteria);
				if($statistic != null) {
					if(UserStatistics::model()->updateByPk(Yii::app()->session['statistic_current_date'], array($select=>$statistic->$select + 1))) {
						$data=new UserStatisticData;
					...
					$statistic=new UserStatistics;
					$statistic->date_key = Yii::app()->session['statistic_current_date'];
					$statistic->$select = 1;
					if($statistic->save()) {
						$data=new UserStatisticData;
						$data->user_id = $this->user_id;
